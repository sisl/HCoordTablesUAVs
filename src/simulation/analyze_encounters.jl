...
function summarize_encounters!(encounters::Vector{Encounter}; verbose::Int64 = 0)
    ...
    alerts_list = Array(Int64, length(encounters))
    conflicts_list = Array(Int64, length(encounters))
    for (idx, e) in enumerate(encounters)
        ...
        alerts_list[idx] = num_alerts
        conflicts_list[idx] = in_conflict
    ...
    alerts_std_dev = std(alerts_list)
    conflicts_std_dev = std(conflicts_list)
    ...
    if verbose == 1
        ...
        println("Alerts Standard Deviation: $(alerts_std_dev)")
        println("Total Conflicts: $(conflicts)")
        println("Average Conflicts: $(conflicts / length(encounters))")
        println("Conflicts Standard Deviation: $(conflicts_std_dev)")
        ...
        # a separate script
        println("$(length(encounters)) $(conflicts) $(alerts) $(conflicts_std_dev) $(alerts_std_dev)")
    end
end
...
function analyze(encounters::Vector{Encounter})
    summarize_encounters!(encounters, verbose = 0)

    # uncomment this function call to step through an encounter
    # step_through(encounters[1])
    
    for (idx, encounter) in enumerate(encounters)
        plot_encounter_actions(encounter, idx, nmac_only = false, as_tex = true)
    #    plot_encounter_paths(encounter, idx, nmac_only = false, as_tex = true)
    end
end