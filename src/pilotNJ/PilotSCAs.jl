...
# export ind2a, 
export state2gridState, gridState2state, index2state, ind2x
...
function reward(mdp::SCA, istate::Int64, iaction::Int64)
    ...
    action = Action(mdp.actions[iaction])
...
function reward(mdp::SCA, state::State, action::Action)
    ...
    reward -= PenAction * (turnOwnship^2) 
...
function getNextState(state::State, action::Action, sigmaTurnOwnship::Float64 = 0.0, sigmaTurnIntruder::Float64 = 0.0, dt::Float64 = DT)
    ...
    if !state.clearOfConflict
       ...
       #todo compare this to the value that ACASXu uses!!
       turnIntruder = sigmaTurnIntruder
...
# This function norms an angle to between 0 and 360 unless the flag neg_pi_to_pi is set to true, in which case it norms it to between -180 and 180
...
function sigmaBank(mdp::SCA, state::State, action::Action)
    sigmaBankValO = SigmaBankO
    sigmaBankValI = SigmaBankI
    ...
    # negative sigma
    negIndicesOwnship, negProbsOwnship = nextStatesSigma(mdp, state, action, -sigmaBankValO, 0.0)

    # positive sigma
    posIndicesOwnship, posProbsOwnship = nextStatesSigma(mdp, state, action, sigmaBankValO, 0.0)

    sigmaBankVal = SigmaBankCOC

    # negative sigma
    negIndicesIntruder, negProbsIntruder = nextStatesSigma(mdp, state, action, 0.0, -sigmaBankValI)

    # positive sigma
    posIndicesIntruder, posProbsIntruder = nextStatesSigma(mdp, state, action, 0.0, sigmaBankValI)

    return [negIndicesOwnship; posIndicesOwnship; negIndicesIntruder; posIndicesIntruder], 
           [negProbsOwnship; posProbsOwnship; negProbsIntruder; posProbsIntruder]
end # function sigmaBank

function nextStatesSigma(mdp::SCA, state::State, action::Action, sigmaTurnOwnship::Float64 = 0.0, sigmaTurnIntruder::Float64 = 0.0)
